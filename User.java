 ...
 *  a list of user names that s/he follows, and the list's size. */
 public class User {
    ...
    public boolean addFollowee(String name) {
        ...
        if (fCount == maxfCount) {
            System.out.println(this.name + " cannot follow " + name);
            return false;
         }
    ...
    // Close the gap: shift left from index
        for (int i = index; i < fCount - 1; i++) {
             follows[i] = follows[i + 1];
         }

    // Clear last slot and shrink logical size
    ...
    public int countMutual(User other) {
        ...
        for (int i = 0; i < fCount; i++) {
        // follows[i] is one of "my" followees
